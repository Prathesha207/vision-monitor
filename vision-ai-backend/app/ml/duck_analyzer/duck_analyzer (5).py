...
# ---------------------------------------------------------------------- #
def appearance_signature(crop, embedder=None):
    """Return a dict signature for a crop, or None.
    Always includes {hist, lab, ar}. If a DuckEmbedder is supplied and active,
    also includes a deep unit-norm 'emb' vector, which signature_sim will
    prefer -- far more discriminative between similar-coloured ducks."""
    if crop is None or crop.size == 0:
        return None
    ...
    sig = {"hist": hist, "lab": lab, "ar": float(ar), "emb": None}
    if embedder is not None:
        try:
            sig["emb"] = embedder.embed(crop)
        except Exception:
            sig["emb"] = None
    return sig
...
def signature_sim(s1, s2):
    """Appearance similarity in [0,1] from two signatures.
    If BOTH signatures carry a deep embedding ('emb'), use its cosine (mapped
    to [0,1]) -- this is the discriminative path that separates similar-coloured
    ducks. Otherwise fall back to the blended colour histogram + Lab + aspect."""
    if s1 is None or s2 is None:
        return 0.0
    # --- preferred: deep-embedding cosine ---
    e1 = s1.get("emb") if isinstance(s1, dict) else None
    e2 = s2.get("emb") if isinstance(s2, dict) else None
    if e1 is not None and e2 is not None:
        # both unit-norm; cosine in [-1,1] -> [0,1]
        cos = float(np.dot(e1, e2))
        return max(0.0, min(1.0, (cos + 1.0) / 2.0))
    # --- fallback: colour histogram + Lab + aspect blend ---
...
class DuckAnalyzer:
    def __init__(self, config_path, expected_duck_count=None):
        ...
        # Deep appearance embedding for re-ID. When on, a SigLIP/ResNet vector
        # replaces the colour histogram as the appearance cue (far better at
        # telling similar-coloured ducks apart). Off by default so nothing
        # changes unless you opt in. 'embedder_backend': auto|siglip|resnet.
        self.use_embedder = bool(cfg.get("use_embedder", False))
        self.embedder_backend = str(cfg.get("embedder_backend", "auto"))
        ...
        # ---- optional deep appearance embedder (re-ID) ----
        # Loaded once, reused for every crop. If unavailable or off, self.embedder
        # stays None and appearance falls back to the colour histogram exactly
        # as before -- so this is fully backward compatible.
        self.embedder = None
        if self.use_embedder:
            try:
                from duck_embedder import DuckEmbedder
                self.embedder = DuckEmbedder(device=self._device_str,
                                             backend=self.embedder_backend)
                if self.embedder.backend is None:
                    self.embedder = None  # nothing loaded -> use colour hist
            except Exception as e:
                print(f"[EMB] [WARN] could not init embedder: {e}")
                self.embedder = None

        # State plainly which appearance cue is in force, so you can confirm at a
        # glance whether the embedder is really being used or it fell back.
        if self.embedder is not None:
            print(f"[EMB] APPEARANCE MODE = deep-embedding ({self.embedder.backend})")
        elif self.use_embedder:
            print("[EMB] APPEARANCE MODE = colour-histogram "
                  "(embedder requested but NOT active -> check pip install)")
        else:
            print("[EMB] APPEARANCE MODE = colour-histogram (embedder off)")
        # counters so a per-run summary can prove which path actually ran
        self._emb_match_count = 0
        self._hist_match_count = 0
    ...
    def _update_slot(self, did, crop, xyxy):
        ...
        sig = appearance_signature(crop, self.embedder)
        ...
        if sig is not None:
            if prev is None:
                ...
                # EMA the deep embedding too, then renormalize so it stays a
                # unit vector (cosine compares stay well-scaled). Keeps a stable
                # identity template rather than the latest possibly-noisy frame.
                pe, se = prev.get("emb"), sig.get("emb")
                if pe is not None and se is not None:
                    m = a * pe + (1 - a) * se
                    n = np.linalg.norm(m)
                    ema["emb"] = (m / n) if n > 0 else se
                elif se is not None:
                    ema["emb"] = se
    ...
    def process_frame(self, frame):
        ...
        for xyxy, tid, crop in pending:
            pend_meta.append((xyxy, tid, crop, center(xyxy), appearance_signature(crop, self.embedder)))
        ...
        for pi, (xyxy, tid, crop, dc, dsig) in enumerate(pend_meta):
            for sj, d in enumerate(free_slots):
                ...
                _tmpl = info.get("sig_ema") or info.get("last_sig")
                ap = signature_sim(dsig, _tmpl)
                # record which appearance path was actually used, so the run can
                # PROVE whether the deep embedding is doing the work or it fell
                # back to the colour histogram.
                if (dsig is not None and _tmpl is not None
                        and dsig.get("emb") is not None
                        and _tmpl.get("emb") is not None):
                    self._emb_match_count += 1
                    _used = "emb"
                else:
                    self._hist_match_count += 1
                    _used = "hist"
                if self.debug:
                    print(f"[DBG f{self.frame_idx}] match tid={tid} slot#{d} "
                          f"appsim={ap:.3f} via={_used}")
                app_matrix[pi, sj] = ap
        ...
        for pi, (xyxy, tid, crop, dc, dsig) in enumerate(pend_meta):
            ...
            if reject:
                # A rejected/pending duck is a candidate EXTRA duck (beyond the
                # anchor set). Regardless of `strict_count`, once it persists
                # for `new_id_patience` frames it should be assigned its own
                # stable display id (e.g. #18 when expected=17) instead of
                # staying an unbound id:-1 box forever. `strict_count` still
                # controls whether the raw too_many_ducks count check fires on
                # the smoothed total; it should not suppress giving this duck
                # an identity in the output.
                ...
                if self.prov_new[tid] >= self.new_id_patience:
                    ...
                    self.tid_to_display[tid] = did
                    _sig = appearance_signature(crop, self.embedder)
                    self.display_info[did] = {"last_crop": crop,
                                              "last_hist": color_hist(crop),
                                              "last_sig": _sig,
                                              "sig_ema": _sig,
                    ...
                    # still within the patience window -> report as unbound
                    # (id:-1) for now; it will pick up a real id once patience
                    # is met above.
        ...
        # Per-duck colouring instead of one colour for the whole frame: only
        # the box(es) actually causing the anomaly should be RED. A duck bound
        # to one of the original `num_anchor` anchor slots is normal (GREEN)
        # even if some OTHER duck elsewhere in the frame is the problem. Only
        # an "excess" duck (a display id beyond num_anchor -- i.e. the extra
        # 18th duck when expected=17), an unconfirmed/unbound extra, a missing
        # duck's frozen box, or a non-duck "other" object are drawn RED. The
        # banner still reflects the overall frame verdict.
        banner_color = RED if is_anomaly else GREEN

        detections = []
        for did, (xyxy, cf) in present_display.items():
            x1, y1, x2, y2 = [int(v) for v in xyxy]
            is_excess = did > self.num_anchor
            dcolor = RED if is_excess else GREEN
            detections.append({"bbox": [x1, y1, x2, y2], "id": int(did),
                               "species": "duck", "confidence": round(float(cf), 4),
                               "status": "excess" if is_excess else "present"})
            draw_box(annotated, xyxy, f"#{did} {int(cf * 100)}%", dcolor)
        ...
        for xyxy, cf in unbound:
            ...
            detections.append({"bbox": [x1, y1, x2, y2], "id": -1,
                               "species": "duck", "confidence": round(float(cf), 4),
                               "status": "unbound"})
            # an unbound duck is, by definition, extra/unexplained -> RED
            draw_box(annotated, xyxy, "duck", RED)
        ...
        for oid, (xyxy, cf) in present_other.items():
            ...
            detections.append({"bbox": [x1, y1, x2, y2], "id": int(oid),
                               ...
                               "status": "present"})
            draw_box(annotated, xyxy, f"other #{oid} {int(cf * 100)}%", RED)
            ...
            if lx:
                ...
                if did in self.missing_active:
                    draw_box(annotated, [x, y, x + w, y + hh],
                             f"#{did} MISSING", RED, dashed=True)

        draw_banner(annotated, status, banner_color)
...
if __name__ == "__main__":
    ...
    if video:
        ...
        while True:
            ...
            # print EVERY frame (was: every 30th)
        ...
        # PROOF of what actually ran: how many appearance comparisons used the
        # deep embedding vs the colour-histogram fallback across the whole video.
        e = analyzer._emb_match_count
        hh = analyzer._hist_match_count
        tot = e + hh
        if tot:
            print(f"[EMB] appearance comparisons: {e} via embedding, "
                  f"{hh} via colour-hist ({100*e/tot:.0f}% embedding)")
        else:
            print("[EMB] no rebind comparisons ran (tracker held every duck) -- "
                  "appearance cue was not exercised this clip")
    else:
